...
public enum DoorVariant {
	 ...
	 *
	 * <p>Doors only now. A trapdoor is too short for a window to be anything but all of it, so the
	 * full-height trapdoor was the glass one twice; it stays registered for the ones already built,
	 * is made no more, and drops the glass trapdoor when broken.
	 */
	FULL_GLASS("full_glass_", false),

	/** Barred the whole way down: a gate you can see through, not a door with a window in it. Doors only, as above. */
