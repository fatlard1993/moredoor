...
def build_icon():
    """One door, split down the middle: solid on the left, barred on the right.

    Three variants side by side sounds like the better picture and is not - at sixteen pixels a
    third of a door is five pixels wide, which is not enough of a door to recognise. Two halves of
    one door still says the door changed, and stays legible.
    """
    ground = [[(28, 24, 20, 255)] * 16 for _ in range(16)]
    solid = read(os.path.join(BLOCK, "oak_door_bottom.png"))
    barred = read(os.path.join(BLOCK, "oak_barred_door_top.png"))

    for y in range(16):
        for x in range(16):
            px = (solid if x < 8 else barred)[y][x]
            if px[3]:
                ground[y][x] = px
    return scale(ground, 8)
